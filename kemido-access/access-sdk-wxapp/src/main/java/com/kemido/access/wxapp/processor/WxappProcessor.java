...
//                WxMaDefaultConfigImpl config = new WxMaRedisConfigImpl(new JedisPool());
        ...
        log.info("[Kemido] |- Bean [kemido Weixin Mini App] Auto Configure.");
    }
    ...
    public WxMaService getWxMaService() {
        ...
        if (StringUtils.isBlank(appId)) {
            log.error("[Kemido] |- Must set [kemido.platform.social.wxapp.default-app-id] property, or use getWxMaService(String appid)!");
            throw new IllegalArgumentException("Must set [kemido.platform.social.wxapp.default-app-id] property");
        }
    ...
    private WxMaJscode2SessionResult getSessionInfo(String code, WxMaService wxMaService) {
        try {
            ...
            log.debug("[Kemido] |- Weixin Mini App login successfully!");
            return sessionResult;
        } catch (WxErrorException e) {
            log.error("[Kemido] |- Weixin Mini App login failed! For reason: {}", e.getMessage());
            return null;
        }
    }
    ...
    private boolean checkUserInfo(String sessionKey, String rawData, String signature, WxMaService wxMaService) {
        if (wxMaService.getUserService().checkUserInfo(sessionKey, rawData, signature)) {
            log.debug("[Kemido] |- Weixin Mini App user info is valid!");
            return true;
        } else {
            log.warn("[Kemido] |- Weixin Mini App user check failed!");
            return false;
        }
    }
    ...
    private WxMaUserInfo getUserInfo(String sessionKey, String encryptedData, String iv, WxMaService wxMaService) {
        ...
        log.debug("[Kemido] |- Weixin Mini App get user info successfully!");
        return wxMaUserInfo;
    }
    ...
    private WxMaPhoneNumberInfo getPhoneNumberInfo(String sessionKey, String encryptedData, String iv, WxMaService wxMaService) {
        log.info("[Kemido] |- Weixin Mini App get encryptedData： {}", encryptedData);
        ...
        try {
            ...
            log.debug("[Kemido] |- Weixin Mini App get phone number successfully!");
            log.debug("[Kemido] |- WxMaPhoneNumberInfo : {}", wxMaPhoneNumberInfo.toString());
            return wxMaPhoneNumberInfo;
        } catch (Exception e) {
            log.error("[Kemido] |- Weixin Mini App get phone number failed!");
            return null;
        }
    }
    ...
    public WxMaJscode2SessionResult login(String code, String appId) {
        ...
        if (StringUtils.isNotBlank(code) && ObjectUtils.isNotEmpty(wxMaService)) {
            ...
            log.error("[Kemido] |- Weixin Mini App login failed, please check code param!");
            return null;
        }
    }
    ...
    public WxMaUserInfo getUserInfo(String appId, String sessionKey, String encryptedData, String iv, String rawData, String signature) {
        ...
        if (ObjectUtils.isNotEmpty(wxMaService)) {
            ...
            log.error("[Kemido] |- Weixin Mini App get user info failed!");
            return null;
        }
    }
    ...
    public WxMaPhoneNumberInfo getPhoneNumberInfo(String appId, String sessionKey, String encryptedData, String iv, String rawData, String signature) {
        ...
        if (ObjectUtils.isNotEmpty(wxMaService)) {
            ...
            log.error("[Kemido] |- Weixin Mini App get phone number info failed!");
            return null;
        }
    }
    ...
    public boolean sendSubscribeMessage(String appId, WxMaSubscribeMessage subscribeMessage) {
        try {
            ...
            log.debug("[Kemido] |- Send Subscribe Message Successfully!");
            return true;
        } catch (WxErrorException e) {
            log.debug("[Kemido] |- Send Subscribe Message Failed!", e);
            return false;
        }
    }
    ...
    public boolean checkMessage(String appId, String message) {
        try {
            ...
            log.debug("[Kemido] |- Check Message Successfully!");
            return true;
        } catch (WxErrorException e) {
            log.debug("[Kemido] |- Check Message Failed!", e);
            return false;
        }
    }
    ...
    public boolean checkImage(String appId, String fileUrl) {
        try {
            ...
            log.debug("[Kemido] |- Check Image Successfully!");
            return true;
        } catch (WxErrorException e) {
            log.debug("[Kemido] |- Check Image Failed! Detail is ：{}", e.getMessage());
            return false;
        }
    }
    ...
    public WxMaMediaAsyncCheckResult mediaAsyncCheck(String appId, String mediaUrl, int mediaType) {
        ...
        try {
            ...
            log.debug("[Kemido] |- Media Async Check Successfully!");
        } catch (WxErrorException e) {
            log.debug("[Kemido] |- Media Async Check Failed! Detail is ：{}", e.getMessage());
        }
